...
exports.getWorkoutAnalytics = asyncHandler(async (req, res) => {
    ...
    // 1b. Weekly Stats Array (Last 7 days duration/calories burned for Dashboard rings)
    const weeklyStats = [];
    for (let i = 0; i < 7; i++) {
      const d = new Date(sevenDaysAgo);
      d.setDate(d.getDate() + i);
      ...
      dayWorkouts.forEach(w => {
        ...
        calories += Math.round((w.duration || 30) * 8.5);
      });
    ...
    const calculateStats = (wList) => {
      ...
      let volume = 0;
      wList.forEach(w => {
        uniqueWorkouts.add(getLocalDateString(w.date));
        sets += w.sets || 0;
        volume += (w.sets || 0) * (w.reps || 0) * (w.weight || 0);
      });
      return { count: uniqueWorkouts.size, sets, volume };
    };
    ...
    const todayWorkoutsFiltered = workouts.filter(w => new Date(w.date) >= startOfToday);
    const caloriesBurnedToday = todayWorkoutsFiltered.reduce((sum, w) => sum + Math.round((w.duration || 30) * 8.5), 0);
    ...
    res.json({
      weeklyStats, // 7-day array of duration & calories burned
      thisWeekSummary,
      lastWeekStats,
